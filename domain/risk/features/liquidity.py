...
def _liquidity_core(free_float_pct: float, dollar_float: float) -> float:
    ...
    f_liq = (
        DEFAULT_ALPHA_FREE_FLOAT * ff_component
        + DEFAULT_ALPHA_DOLLAR_FLOAT * dv_component
    )
...
def compute_liquidity_features(ipo: IpoInput) -> Dict[str, float]:
    ...
    free_float_fraction = min(
        max(ipo.deal_terms.free_float_pct, 0.0) / 100.0, 1.0
    )
    ...
    f_liq_total = (
        DEFAULT_WEIGHT_LIQUIDITY * f_liq
        + DEFAULT_WEIGHT_LOCKUP * f_lock
    )
